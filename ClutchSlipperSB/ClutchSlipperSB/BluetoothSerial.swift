...
/// Global serial handler, don't forget to initialize it with init(delgate:) var is used to declare a variable in swift with the name "serial". The type of variable is "BluetoothSerial" which is the class defined earlier (file name.) The ! at the end means that the variable is an implicitely unwrapped optional. An implicitley unwrapped optional is a special type in swift that can hold either a value of a certain type or nil. The ! indicates that I can expect the variable to always hold a vlue and I can use it without unwrapping it explicitly. If I try to access it when it is nil, the program will crash.
var serial: BluetoothSerial!
// Delegate functions
/// In swift, a protocol is a blueprint of methods (functions), properties, and other requirements that can be adopted by a class, struct or enum to provide specific functionality. In this case the : class specifies that the protocol can only be adopted by class types
...
final class BluetoothSerial: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var serviceUUID = CBUUID(string: "FFE0") // This is specific to the HM-10 BT module I am using
    
    /// UUID of the characteristic to look for.
    var characteristicUUID = CBUUID(string: "FFE1") // This is specific to the HM-10 BT module I am using
